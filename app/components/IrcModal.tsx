...
type IrcResult = { status: "confirmed"; rank: number } | { status: "waitlisted"; waitNumber: number };

const CAPACITY = 11;
const DISMISS_KEY = "chiro-irc-modal-dismissed-until";
...
function resultText(result: IrcResult): string {
...
export function IrcModal({ autoOpen = false }: { autoOpen?: boolean }) {
  ...
  const [done, setDone] = useState<{ updated: boolean; result: IrcResult } | null>(null);

  const [mineName, setMineName] = useState("");
  const [mineStudentId, setMineStudentId] = useState("");
  const [mineLoading, setMineLoading] = useState(false);
  const [mineError, setMineError] = useState<string | null>(null);
  const [mineResult, setMineResult] = useState<IrcResult | null>(null);

  function loadStats() {
    fetch("/api/irc", { cache: "no-store" })
      .then((res) => res.json())
      .then((data) => {
        if (data?.success) setConfirmedCount(data.confirmedCount ?? 0);
      })
      .catch(() => {});
  }
  ...
  const remaining = confirmedCount !== null ? Math.max(0, CAPACITY - confirmedCount) : null;
          ...
          (done ? (
            ...
            <>
              <p className="mb-5 text-sm font-medium text-slate-500">
                제21회 국제로봇콘테스트(IRC) 참가 신청입니다. 정원 {CAPACITY}명이며
                {remaining !== null && (remaining > 0 ? ` 현재 여석 ${remaining}자리 남았습니다.` : " 정원이 마감되어 예비번호로 등록됩니다.")}
              </p>
