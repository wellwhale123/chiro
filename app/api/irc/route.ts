...
export async function GET() {
  try {
    ...
    return NextResponse.json({ success: true, capacity: IRC_CAPACITY, ...stats });
  } catch (error) {
    console.error("IRC 신청 현황 조회 실패:", error);
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // 신청 시점에 이미 정원(11명)을 넘겼으면(=예비번호가 될 예정) 입금을 요구하지 않습니다.
    const statsBefore = await getIrcStats();
    const willBeConfirmed = statsBefore.confirmedCount < IRC_CAPACITY;
    ...
    const { updated, result } = await submitIrcRegistration(
      name,
      studentId,
      teammateNames,
